...
def load_models():
    try:
        face_model = load_model(r"C:\Users\ROY\Documents\python_proj\Task5\Task5 models\facedetectortask5.h5", custom_objects={'mse': MeanSquaredError()})
        nationality_model = load_model(r"C:\Users\ROY\Documents\python_proj\Task5\Task5 models\nationalitytask5.h5")
        emotion_model = load_model(r"C:\Users\ROY\Documents\python_proj\Task5\Task5 models\emotiontask5.h5")
        clothing_model = load_model(r"C:\Users\ROY\Documents\python_proj\Task5\Task5 models\clothingtask5.h5", custom_objects={'mse': MeanSquaredError()})  
        age_model = load_model(r"C:\Users\ROY\Documents\python_proj\Task5\Task5 models\agetask5.h5")
...
def detect_and_crop_clothing(image, clothing_model):
    try:
        ...
        if not (0 <= cx1 <= 1 and 0 <= cy1 <= 1 and 0 <= cx2 <= 1 and 0 <= cy2 <= 1):
            st.warning("Invalid clothing bounding box coordinates hence, using full image region instead.")
            return img_resized
        if cx2 <= cx1 or cy2 <= cy1:
            st.warning("Invalid clothing bounding box hence, using full image region instead.")
            return img_resized
        orig_height, orig_width = img_array.shape[:2]
        ...
        if cx2 <= cx1 or cy2 <= cy1:
            st.warning("Invalid clothing bounding box after applying buffer hence, using full image region instead.")
            return img_resized
        ...
        if clothing_crop.size == 0:
            st.warning("Empty clothing crop detected hence, using full image region instead.")
            return img_resized
...
def preprocess_nationality_emotion_image(image):
    try:
        ...
        st.error(f"Error in nationality and/or emotion preprocessing: {e}")
        return None
...
def predict_clothing_color(image_features):
    try:
        if image_features.size == 0:
            return "No clothing region detected"
        ...
        if uploaded_files:
            with st.spinner("Analyzing images"):
                for i, uploaded_file in enumerate(uploaded_files):
                    ...
                    if clothing_crop.size == 0:
                        st.warning("Failed to detect clothing region hence, using face region instead.")
